...
void SmartIO_Init(void)
{
    ...
    /* Turn off the DU */
    SmartIO_DU_SEL = SmartIO_INIT_DU_SEL;
    SmartIO_DU_CTL = SmartIO_INIT_DU_CTL;
    SmartIO_DATA = SmartIO_INIT_DU_DATA_REG;
}
...
cystatus SmartIO_DU_SelectTriggers(uint32 tr0Sel, uint32 tr1Sel, uint32 tr2Sel)
{
    ...
    else
    {
        ...
        sel = SmartIO_DU_SEL & SmartIO_DU_TR_CLEAR_ALL;
        SmartIO_DU_SEL = sel 
    ...
    switch(dataNum)
    {
        case SmartIO_DATA0:
            sel = (sel & SmartIO_DU_DATA0_CLEAR)
                  | (uint32)((dataSel << CYFLD_PRGIO_PRT_DU_DATA0_SEL__OFFSET) & SmartIO_DU_DATA0_MASK);
        break;
        case SmartIO_DATA1:
            sel = (sel & SmartIO_DU_DATA1_CLEAR)
                  | (uint32)((dataSel << CYFLD_PRGIO_PRT_DU_DATA1_SEL__OFFSET) & SmartIO_DU_DATA1_MASK);
        break;
...
cystatus SmartIO_DU_OpCode(uint32 opCode, uint32 dataSize)
{
    ...
    else
    {
         /* Must disable the block before configuring */
        SmartIO_Disable();
        SmartIO_DU_CTL = (uint32)(((dataSize - 1u) << CYFLD_PRGIO_PRT_DU_SIZE__OFFSET))
