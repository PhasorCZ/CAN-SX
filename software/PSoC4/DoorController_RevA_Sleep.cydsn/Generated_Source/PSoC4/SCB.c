...
static void SCB_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void SCB_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            SCB_RX_SCL_MOSI_HSIOM_SEL_GPIO,
            SCB_TX_SDA_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (SCB_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_RX_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_PIN_DM_OD_LO;
        #elif (SCB_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[SCB_RX_WAKE_SCL_MOSI_PIN_INDEX] = SCB_RX_WAKE_SCL_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SCB_RX_WAKE_SCL_MOSI_PIN_INDEX] = SCB_PIN_DM_OD_LO;
        #else
        #endif /* (SCB_RX_SCL_MOSI_PIN) */
        
        #if (SCB_TX_SDA_MISO_PIN)
            hsiomSel[SCB_TX_SDA_MISO_PIN_INDEX] = SCB_TX_SDA_MISO_HSIOM_SEL_I2C;
            pinsDm  [SCB_TX_SDA_MISO_PIN_INDEX] = SCB_PIN_DM_OD_LO;
        ...
        {
        #if (SCB_RX_SCL_MOSI_PIN)
            hsiomSel[SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_RX_SCL_MOSI_HSIOM_SEL_SPI;
        #elif (SCB_RX_WAKE_SCL_MOSI_PIN)
            hsiomSel[SCB_RX_WAKE_SCL_MOSI_PIN_INDEX] = SCB_RX_WAKE_SCL_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (SCB_RX_SCL_MOSI_PIN) */
        
        #if (SCB_TX_SDA_MISO_PIN)
            hsiomSel[SCB_TX_SDA_MISO_PIN_INDEX] = SCB_TX_SDA_MISO_HSIOM_SEL_SPI;
        #endif /* (SCB_TX_SDA_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_PIN_DM_DIG_HIZ;
                pinsDm[SCB_TX_SDA_MISO_PIN_INDEX] = SCB_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= SCB_TX_SDA_MISO_PIN_MASK;
            #endif /* (SCB_TX_SDA_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_PIN_DM_STRONG;
                pinsDm[SCB_TX_SDA_MISO_PIN_INDEX] = SCB_PIN_DM_DIG_HIZ;
            ...
            #if (SCB_RX_SCL_MOSI_PIN)
                pinsInBuf |= SCB_RX_SCL_MOSI_PIN_MASK;
            #elif (SCB_RX_WAKE_SCL_MOSI_PIN)
                pinsInBuf |= SCB_RX_WAKE_SCL_MOSI_PIN_MASK;
            #else
            #endif /* (SCB_RX_SCL_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (SCB_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (SCB_TX_SDA_MISO_PIN)
                hsiomSel[SCB_TX_SDA_MISO_PIN_INDEX] = SCB_TX_SDA_MISO_HSIOM_SEL_UART;
                pinsDm  [SCB_TX_SDA_MISO_PIN_INDEX] = SCB_PIN_DM_OD_LO;
            #endif /* (SCB_TX_SDA_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (SCB_RX_SCL_MOSI_PIN)
                    hsiomSel[SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_RX_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SCB_RX_SCL_MOSI_PIN_INDEX] = SCB_PIN_DM_DIG_HIZ;
                #elif (SCB_RX_WAKE_SCL_MOSI_PIN)
                    hsiomSel[SCB_RX_WAKE_SCL_MOSI_PIN_INDEX] = SCB_RX_WAKE_SCL_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SCB_RX_WAKE_SCL_MOSI_PIN_INDEX] = SCB_PIN_DM_DIG_HIZ;
                #else
                #endif /* (SCB_RX_SCL_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        SCB_SET_HSIOM_SEL(SCB_RX_SCL_MOSI_HSIOM_REG,
                                       ...
                                       SCB_RX_SCL_MOSI_HSIOM_POS,
                                        hsiomSel[SCB_RX_SCL_MOSI_PIN_INDEX]);

        SCB_uart_rx_i2c_scl_spi_mosi_SetDriveMode((uint8) pinsDm[SCB_RX_SCL_MOSI_PIN_INDEX]);

        #if (!SCB_CY_SCBIP_V1)
            SCB_SET_INP_DIS(SCB_uart_rx_i2c_scl_spi_mosi_INP_DIS,
                                         SCB_uart_rx_i2c_scl_spi_mosi_MASK,
                                         (0u != (pinsInBuf & SCB_RX_SCL_MOSI_PIN_MASK)));
        #endif /* (!SCB_CY_SCBIP_V1) */
        ...
        SCB_SET_HSIOM_SEL(SCB_RX_WAKE_SCL_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[SCB_RX_WAKE_SCL_MOSI_PIN_INDEX]);

        SCB_uart_rx_wake_i2c_scl_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[SCB_RX_WAKE_SCL_MOSI_PIN_INDEX]);

        SCB_SET_INP_DIS(SCB_uart_rx_wake_i2c_scl_spi_mosi_INP_DIS,
                                     SCB_uart_rx_wake_i2c_scl_spi_mosi_MASK,
                                     (0u != (pinsInBuf & SCB_RX_WAKE_SCL_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        SCB_SET_INCFG_TYPE(SCB_RX_WAKE_SCL_MOSI_INTCFG_REG,
                                        SCB_RX_WAKE_SCL_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (SCB_TX_SDA_MISO_PIN)
        SCB_SET_HSIOM_SEL(SCB_TX_SDA_MISO_HSIOM_REG,
                                       SCB_TX_SDA_MISO_HSIOM_MASK,
                                       SCB_TX_SDA_MISO_HSIOM_POS,
                                        hsiomSel[SCB_TX_SDA_MISO_PIN_INDEX]);

        SCB_uart_tx_i2c_sda_spi_miso_SetDriveMode((uint8) pinsDm[SCB_TX_SDA_MISO_PIN_INDEX]);

    #if (!SCB_CY_SCBIP_V1)
        SCB_SET_INP_DIS(SCB_uart_tx_i2c_sda_spi_miso_INP_DIS,
                                     SCB_uart_tx_i2c_sda_spi_miso_MASK,
                                    (0u != (pinsInBuf & SCB_TX_SDA_MISO_PIN_MASK)));
    #endif /* (!SCB_CY_SCBIP_V1) */
    #endif /* (SCB_RX_SCL_MOSI_PIN) */
