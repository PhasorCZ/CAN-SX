...
/*******************************************************************************
* Function Name: SCB_SpiCyBtldrCommStart
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SCB_SpiCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SCB_SpiCyBtldrCommReset
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SCB_SpiCyBtldrCommRead
****************************************************************************//**
*
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        do
        {
            ...
            if (0u != SCB_SpiUartGetRxBufferSize())
            {
                ...
                do
                {
                    byteCount = SCB_SpiUartGetRxBufferSize();
                    CyDelayUs(SCB_SPI_BYTE_TO_BYTE);
                }
                while (byteCount != SCB_SpiUartGetRxBufferSize());

                /* Disable data reception into RX FIFO */
                SCB_RX_FIFO_CTRL_REG |= SCB_RX_FIFO_CTRL_FREEZE;

                byteCount = SCB_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) SCB_SpiUartReadRxData();
        }
    }
...
/*******************************************************************************
* Function Name: SCB_SpiCyBtldrCommWrite
****************************************************************************//**
*
*  Allows the caller to read data from the bootloader host (the host writes the
*  data). The function handles polling to allow a block of data to be completely
...
cystatus SCB_SpiCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        SCB_SpiUartPutArray(pData, (uint32) size);

        /* Wait with timeout 1mS for packet end */
        do
        {
            /* Check for packet end */
            if ((0u == SCB_SpiUartGetTxBufferSize()) &&
                (0u == (SCB_SPI_STATUS_REG & SCB_SPI_STATUS_BUS_BUSY)) &&
                (0u == (SCB_TX_FIFO_STATUS_REG & SCB_TX_FIFO_SR_VALID)))
            {
                *count = size;
                status = CYRET_SUCCESS;
        ...
        SCB_SpiUartClearRxBuffer();
        SCB_RX_FIFO_CTRL_REG &= (uint32) ~SCB_RX_FIFO_CTRL_FREEZE;
    }
